              ...
              <div class="card-body">
                  <table id="crud" class="table table-striped" style="width:100%">
                    ...
                    <?php while ($preg = $pregunta->fetch_object()):?>
                        <tr>
                            <td><?=$preg->idcuestionariog?></td>
                            <td><?=$preg->pregunta?></td>

                            <?php $bloque = Utils::showBloqueCompetenciaById($preg->idbloque);?>
                            <td><?=$bloque->bloquecompetencia?></td>
                              ...
                              <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                                <button type="button" class="btn btn-secondary btn-flat dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">Acción</button>
                                  <ul class="dropdown-menu">
                                    <a id="editmodal" class="dropdown-item editbtn" data-bs-toggle="modal" data-bs-target="#editPreguna"><li class="fa fa-fw fa-eye"></li> Editar</a>
                                    <?php if($preg->idstatus == 1):?>
                                    <a class="dropdown-item" href="<?=baseUrl?>?controller=cuestionariogen&action=desactivarPregunta&id=<?=$preg->idcuestionariog?>&evaid=<?=$evaid?>"><i class="fa fa-fw fa-trash"></i> Desactivar</a>
                                    <?php elseif($preg->idstatus == 2):?>
                                    <a class="dropdown-item" href="<?=baseUrl?>?controller=cuestionariogen&action=desactivarPregunta&id=<?=$preg->idcuestionariog?>&evaid=<?=$evaid?>"><i class="fas fa-check"></i> Activar</a>
                                    <?php endif;?>
                                  </ul>
                              </div>
                            </td>
